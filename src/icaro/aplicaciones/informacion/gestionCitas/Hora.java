...
public class Hora implements Comparable, Cloneable, java.io.Serializable {
	...
	/**
	 * Momento del d�a. Puede ser: MOMENTO_DIA_AM, MOMENTO_DIA_PM, o
	 * indefinido
	 */
	 ...
	 * @param h
	 *            Hora
	 * @return n�mero de ms transcurridos desde 1-Enero-1970, 00:00:00 GMT + 1
	 *         (Europa/Madrid)
	 */
	...
	public static Hora getHoraFromMs(long ms) {
		...
		return new Hora(cal.get(Calendar.HOUR_OF_DAY),
	...
	public static SimpleTimeZone getSimpleTimeZoneMadrid() {
		...
		SimpleTimeZone stz = new SimpleTimeZone(+1 * 60 * 60 * 1000,
				"Europe/Madrid");

		// Creamos reglas de cambio de hora "Daylight saving time-adjustment"
		// (DST)

		// DST started on Sunday March 31, 2002 at 2:00:00 AM local standard
		// time
	 ...
	 * @param ms
	 *            n�mero de ms desde las 0 horas del 1 de enero de 1970
	 * @return Array de 2 componentes, componente [0] = Fecha componente [1] =
	 *         Hora
	 */
	...
	/**
	 * Haya la diferencia de tiempos entre la hora actual y la hora dada y la
	 * devuelve en un nuevo objeto hora
	 *
	 * @param hora
	...
	/**
	 * Comprueba si la hora est� indefinida, es decir, si ninguno de sus
	...
	/**
	 * Convierte una hora con el siguiente formato: "hh:mm:ss" o "hh:mm" o
	 * "h:mm" o "h:mm:ss" en un objeto de la clase hora
	 *
	 * @param horaS
	 *            cadena que contiene la hora
	 * @return objeto de la clase Hora equivalente a la hora dada en forma de
	 *         cadena
	 */
	public static Hora string2Hora(String horaS) {
		// La hora en cadena tiene que tener el siguiente formato: "hh:mm:ss" o
		// "hh:mm" o "h:mm" o "h:mm:ss"
		Hora hora = new Hora();
		try {
			...
			if (segundoSeparador > -1) {
				...
				minutos = horaS
						.substring(primerSeparador + 1, segundoSeparador);
			} else {
			...
			if (segundoSeparador > -1) {
				// Hay segundo separador
				segundos = horaS
						.substring(segundoSeparador + 1, horaS.length());
			}

			hora = new Hora(Integer.parseInt(hora2S),
	...
	/**
	 * Devuelve un objeto de la clase Date con la fecha del sistema y la hora
	 * dada por el objeto actual
	 *
	...
	public Date toDate(Date fecha) {
		// Devuelve la fecha pasada por par�metro pero actualizada a la nueva
		// hora
		...
		int dia = calendar.get(Calendar.DATE);
		// Actualizamos los nuevos campos
		calendar.set(anyo, mes, dia, this.getHora(), this.minutos,
				this.segundos);
	 ...
	 *         tiene un n�mero de campos definidos igual o mayor que la hora
	...
	public void completar(Vector listaSugerencias) {
		...
		// Comprobamos si la hora obtenida completando con la lista de
		// sugerencias
		...
		if (horaCompletada.estaCompleta()) {
			if (horaCompletada.horaEnHorarioLaboral()) {
				...
				// Completamos la hora con la hora obtenida a partir de la lista
				// de
				// sugerencias
				this.completar(horaCompletada);
			} else {
				// La fecha no est� dentro del horario laboral. Completamos la
				// hora con sentido com�n
				this.completar();
			}
		} else {
	 ...
	 *         actual y la hora es menor que la hora actual y false en caso
	 *         contrario
	 */
	public boolean horaCorrecta(Fecha fecha) {
		...
		if ((fecha != null) && (this != null)) {
			...
			// La hora del aviso es mayor o igual que la hora actual si el
			// resultado de la
	...
	public boolean horaEnHorarioLaboral() {
		...
		// int ultimaHoraCita =
		// Integer.parseInt(Configuracion.obtenerParametro("HORA_FIN_CITAS"));
	...
	public static void main(String args[]) {
		 ...
		 * 
		 * if(h1.compareTo(h2)<0) System.out.println("menor"); else
		 * if(h1.compareTo(h2)>0) System.out.println("mayor"); else
		 * if(h1.compareTo(h2)==0) System.out.println("igual");
