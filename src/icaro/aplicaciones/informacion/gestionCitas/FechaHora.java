...
public class FechaHora implements Comparable, Cloneable, java.io.Serializable {
	...
	@Override
	public Object clone() {
		return new FechaHora((Fecha) this.fecha.clone(),
				(Hora) this.hora.clone());
	}
	...
	public void completar(Vector listaSugerencias) {
		...
		if (this.fecha == null) {
			// Si la fecha es indefinida creamos una fecha con todos los campos
			// indefinidos
		...
		if (this.hora == null) {
			// Si la hora es indefinida creamos una hora con todos los campos
			// indefinidos
			// para intentar completar con la lista
			this.hora = new Hora();
		}
		...
		if (this.hora.hora == Utilidades.int_INDEFINIDO) {
			// Las horas son indefinidas y por tanto, debemos dar la hora como
			// indefinida
			this.hora = null;
		}
		if (this.fecha.dia == Utilidades.int_INDEFINIDO) {
			// El dia es indefinido y por tanto, debemos dar la fecha como
			// indefinida
			this.fecha = null;
		}
	}
